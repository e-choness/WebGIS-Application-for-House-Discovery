...
layui
  ...
  .define(['setter', 'admin'], function(exports) {
      ...
      */
        ...
        view()
          ...
          .done(function() {
            ...
            $win.on('resize', layui.sessionData.resize)
          ...
          if (setter.interceptor) {
            ...
            if (!local[setter.request.tokenName]) {
              return (location.hash =
                '/user/login/redirect=' + encodeURIComponent(pathURL)) //跳转到登入页
            }
          }
    ...
    layui.each(setter.extend, function(index, item) {
      ...
      mods[name] = '{/}' + setter.base + 'lib/extend/' + item
      console.log(name)
      layui.extend(mods)
    })

    //扩展 utils 目录下的其它模块
    layui.each(setter.utils, function(index, item) {
      var mods = {}
      var name = item
      if (item.indexOf('/') !== -1) {
        var arr = item.split('/')
      ...
      mods[name] = '{/}' + setter.base + 'utils/' + item
      layui.extend(mods)
    })

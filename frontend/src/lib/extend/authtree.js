...
layui.define(['jquery', 'form'], function(exports) {
  ...
  var obj = {
    ...
    render: function(dst, trees, opt) {
      ...
      // 默认展开有选中数据的层
      var openchecked =
        typeof opt.openchecked !== 'undefined' ? opt.openchecked : true
      opt.openchecked = openchecked
      // 自动取消选中
      var autoclose =
        typeof opt.autoclose !== 'undefined' ? opt.autoclose : true
      opt.autoclose = autoclose
      // 自动选择直属父级节点
      var autochecked =
        typeof opt.autochecked !== 'undefined' ? opt.autochecked : true
      opt.autochecked = autochecked
      // 是否隐藏左侧 单选/多选的选框 -- 特殊需求，一般用于单选树并且不用
      var hidechoose =
        typeof opt.hidechoose !== 'undefined' ? opt.hidechoose : false
      opt.hidechoose = hidechoose
      // 是否开启半选
      var halfchoose =
        typeof opt.halfchoose !== 'undefined' ? opt.halfchoose : false
      opt.halfchoose = halfchoose
      // 收起叶子节点（排列于一行）
      var collapseLeafNode =
        typeof opt.collapseLeafNode !== 'undefined'
          ? opt.collapseLeafNode
          : false
      opt.collapseLeafNode = collapseLeafNode
      // 收起最后一层节点（排列于一行）
      var collapseLastDepthNode =
        typeof opt.collapseLastDepthNode !== 'undefined'
          ? opt.collapseLastDepthNode
          : false
      ...
      // 展开、折叠节点的前显字符配置
      opt.openIconContent = opt.openIconContent
        ? opt.openIconContent
        : '&#xe625;'
      this.openIconContent = opt.openIconContent
      opt.closeIconContent = opt.closeIconContent
        ? opt.closeIconContent
        : '&#xe623;'
      this.closeIconContent = opt.closeIconContent
      // 选中、半选中、未选中节点的图标配置
      opt.checkedIconContent = opt.checkedIconContent
        ? opt.checkedIconContent
        : 'e605'
      this.checkedIconContent = opt.checkedIconContent
      opt.halfCheckedIconContent = opt.halfCheckedIconContent
        ? opt.halfCheckedIconContent
        : 'e605'
      this.halfCheckedIconContent = opt.halfCheckedIconContent
      opt.notCheckedIconContent = opt.notCheckedIconContent
        ? opt.notCheckedIconContent
        : '&#xe605;'
      ...
      var timer = 0
      $(dst)
        .find('.auth-single:first')
        .unbind('click')
        .on('click', '.layui-form-checkbox,.layui-form-radio', function(event) {
            ...
            if (autochecked) {
              if (checked) {
                ...
                elem
                  .parents('.auth-child')
                  .prev()
                  .find('.authtree-checkitem:not(:disabled)[type="checkbox"]')
                  .prop('checked', true)
              }
              elem
                .parent()
      ...
      $(dst)
        .find('.auth-single:first')
        .unbind('dblclick')
        .on('dblclick', '.layui-form-checkbox,.layui-form-radio', function(e) {
          // 触发时间 > 0，才触发双击事件
    ...
    _autoclose: function(obj) {
      ...
      if (
        single.find(
          'div>.auth-status>input.authtree-checkitem:not(:disabled)[type="checkbox"]:checked'
        ).length === 0
      ) {
        authStatus
          .find('.authtree-checkitem:not(:disabled)[type="checkbox"]')
    ...
    renderAuth: function(tree, dept, opt) {
      ...
      layui.each(tree, function(index, item) {
        var hasChild =
          item[childKey] &&
          (item[childKey].length || !$.isEmptyObject(item[childKey].length))
            ? 1
            : 0
        // 注意：递归调用时，this的环境会改变！
        var append = hasChild
          ? obj.renderAuth(item[childKey], dept + 1, opt)
          : ''
        var openstatus = openall || (opt.openchecked && item.checked)
        var isChecked = _this._getStatusByDynamicKey(
          item,
          opt.checkedKey,
          opt.valueKey
        )
        var isDisabled = _this._getStatusByDynamicKey(
          item,
          opt.disabledKey,
          opt.valueKey
        )
        ...
        if (rowFlag) {
          str +=
            '<div class="auth-row auth-skin"><div class="auth-row-item auth-status" style="display: flex;flex-direction: row;align-items: flex-end;">' +
            (hasChild
              ? ''
              : '<i class="layui-icon auth-leaf" style="opacity:0;color: transparent;">&#xe626;</i>')
        } else {
          // '+new Array(dept * 4).join('&nbsp;')+'
          str +=
            '<div class="auth-skin"><div class="auth-status" style="display: flex;flex-direction: row;align-items: flex-end;"> ' +
            (hasChild
              ? '<i class="layui-icon auth-icon ' +
                (openstatus ? 'active' : '') +
                '" style="cursor:pointer;">' +
                (openstatus ? obj.openIconContent : obj.closeIconContent) +
                '</i>'
              : '<i class="layui-icon auth-leaf" style="opacity:0;color: transparent;">&#xe626;</i>') +
            (dept > 0
              ? '<span class="auth-prefix">' + opt.prefixChildStr + ' </span>'
              : '')
        }
        str +=
          '<input class="authtree-checkitem authtree-dept-' +
          dept +
          ' ' +
          (dept === opt.treeDept ? 'authtree-dept-max' : '') +
          '" type="' +
          opt.checkType +
          '" name="' +
          inputname +
    ...
    showChecked: function(dst) {
      $(dst)
        .find('.authtree-checkitem:checked')
        .parents('.auth-child')
        .show()
    },
    /**
      ...
      for (var index in list) {
        if (list.hasOwnProperty(index)) {
          ...
          if (
            ...
            // 满足条件则递归
            var nextChild = this._listToTree(
              list,
              item[opt.primaryKey],
              currentDept + 1,
              opt
            ...
            // 禁用/选中节点的两种渲染方式
            node['checked'] = this._getStatusByDynamicKey(
              item,
              opt.checkedKey,
              opt.valueKey
            )
            node['disabled'] = this._getStatusByDynamicKey(
              item,
              opt.disabledKey,
              opt.valueKey
            )
    ...
    treeConvertSelect: function(tree, opt) {
      ...
      opt.prefixFirstEmpty = opt.prefixFirstEmpty
        ? opt.prefixFirstEmpty
        : '　　'
    ...
    _treeToSelect: function(tree, currentDept, opt) {
      ...
      for (var index in tree) {
        ...
        var item = tree[index]
        if (
          opt.childKey in item &&
          item[opt.childKey] &&
          item[opt.childKey].length > 0
        ) {
          child_flag = 1
        }
        ...
        ansList.push({
          ...
          checked: this._getStatusByDynamicKey(
            item,
            opt.checkedKey,
            opt.valueKey
          ),
          disabled: this._getStatusByDynamicKey(
            item,
            opt.disabledKey,
            opt.valueKey
          )
    ...
    autoWidth: function(dst) {
      ...
      // 自动宽度调整的逻辑
      $(dst)
        .find('.layui-form-checkbox,.layui-form-radio,.layui-form-audio')
        .each(function(index, item) {
          ...
          if ($(this).is(':hidden')) {
            ...
            $('body').append(
              '<div id="layui-authtree-get-width">' + $(this).html() + '</div>'
            )
            width =
              $('#layui-authtree-get-width')
                .find('span')
                .width() +
              $('#layui-authtree-get-width')
                .find('i')
    ...
    autoNodeRender: function(dst) {
      ...
      document.styleSheets[0].addRule(
        dst + ' .layui-icon-ok:before',
        'content: ' + this.checkedIconContent
      )
    },
    ...
    checkAll: function(dst) {
      ...
      origin
        .find('.authtree-checkitem:not(:disabled):not(:checked)')
        .prop('checked', true)
      form.render('checkbox')
      form.render('radio')
      obj.autoWidthAll()
    ...
    uncheckAll: function(dst) {
      var origin = $(dst)
      origin
        .find('.authtree-checkitem:not(:disabled):checked')
        .prop('checked', false)
      form.render('checkbox')
      form.render('radio')
      obj.autoWidthAll()
    ...
    _saveNodeStatus: function(dst) {
      ...
      // 保存新信息前，最新选择的信息
      this.lastCheckedNode[dst] = this._getLastChecked(
        dst,
        currentChecked,
        currentNotChecked
      )
      this.lastNotCheckedNode[dst] = this._getLastNotChecked(
        dst,
        currentChecked,
        currentNotChecked
      )
